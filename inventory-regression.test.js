...
assert.match(source, /@version\s+1\.2\.15/);
...
assert.match(source, /<dt>Layout Profile<\/dt>/);
...
assert.match(source, /function layoutProfile\(\)/);
assert.match(source, /dashboard\.dataset\.runtimeKind = runtime\.runtimeKind/);
assert.match(source, /dashboard\.dataset\.layoutProfile = runtime\.layout/);
assert.match(source, /function inventoryFreshness\(\)/);
assert.match(source, /Inventory data · " \+ freshness\.source/);
assert.match(source, /Refresh inventory/);
assert.match(source, /function standardFeedbackLayer\(\)/);
assert.match(source, /toast\.remove\(\);\s*state\.toastTimers\.delete\(timer\);/);
...
assert.match(source, /minimizedPosition: null/);
assert.match(source, /minimizedPosition: state\.minimizedPosition/);
assert.match(source, /function applyMinimizedPosition\(position = state\.minimizedPosition\)/);
assert.match(source, /function saveMinimizedPosition\(\)/);
assert.match(source, /Tap to open · drag to move/);
assert.match(source, /if \(state\.isMinimized\) saveMinimizedPosition\(\);/);
assert.match(source, /if \(restore\) restoreMinimizedWidget\(\);/);
assert.match(source, /#nic-wrapper\[data-minimized='true'\] #nic-drag\{[^}]*touch-action:none/);
assert.match(source, /dashboard\?\.minimizedPosition && typeof dashboard\.minimizedPosition === "object"/);
assert.match(source, /isStorageRecord\(dashboard\.minimizedPosition\) \? dashboard\.minimizedPosition : null/);
